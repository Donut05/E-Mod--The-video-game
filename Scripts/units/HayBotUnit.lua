...
function HaybotUnit.server_onUnitUpdate(self, dt)
	...
	local combatPathing = self.combatTicks > 0 or (#self.allyUnits >= 2 and self.lastTargetPosition) or
	closestVisibleCrop or closestVisibleWormCharacter
	...
	if combatPathing then
		...
		if nextTargetPosition then
			...
			local valid, breachPosition = sm.ai.getBreachablePosition(self.unit, leveledNextTargetPosition, breachDepth,
				MeleeBreachLevel)
	...
	if (done or abortState) then
		...
		elseif combatPathing then
			...
			if currentTargetPosition then
				...
				local distance = fromToTarget:length()
				local flatCurrentTargetPosition = sm.vec3.new(currentTargetPosition.x, currentTargetPosition.y,
					self.unit.character.worldPosition.z)
...
function HaybotUnit.server_onProjectile(self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal,
										projectileUuid)
...
function HaybotUnit.server_onCollision(self, other, collisionPosition, selfPointVelocity, otherPointVelocity,
									   collisionNormal)
	if not sm.exists(self.unit) then
		return
	end
...
function HaybotUnit.sv_spawnParts(self, impact)
	...
	if not g_disableScrapHarvest then
		...
		local scrapShape = scrapBody:createPart(obj_harvest_metal, sm.vec3.new(-1, 2, -1), sm.vec3.new(0, -1, 0),
