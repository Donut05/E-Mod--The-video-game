	...
	if self.currentState then
		...
		if self.currentState == self.combatFollowAttackState or self.currentState == self.combatChaseAttackState or self.currentState == self.pathingState then
			...
			if self.stompTimer:done() then
				...
				sm.melee.meleeAttack(melee_farmbotstep, 3,
					self.unit.character.worldPosition + self.unit.character.direction * 0.875 -
					sm.vec3.new(0, 0, self.unit.character:getHeight() * 0.375), self.unit.character.direction * 1.75,
					self.unit, 0, 4000)
			end
		else
			self.stompTimer:reset()
		...
		if self.voiceTimer:done() then
			...
			sm.event.sendToCharacter(self.unit.character, "sv_e_characterEffect",
				{ effectName = "Farmbot - Voice", effectParams = effectParams })
		end
	end
...
function FarmbotUnit.server_onUnitUpdate(self, dt)
	...
	if self.lastTargetPosition then
		...
		flatFromToTarget = (flatFromToTarget:length() >= FLT_EPSILON) and flatFromToTarget:normalize() or
			self.unit.character.direction
		local flatDesiredDirection = sm.vec3.new(desiredDirection.x, desiredDirection.y, 0)
		flatDesiredDirection = (flatDesiredDirection:length() >= FLT_EPSILON) and flatDesiredDirection:normalize() or
			self.unit.character.direction
	...
	if self.isInCombat then
		...
		if nextTargetPosition then
			...
			local valid, breachPosition, breachObject = sm.ai.getBreachablePosition(self.unit, leveledNextTargetPosition,
				breachDepth + self.unit.character:getRadius(), MeleeBreachLevel)
			if valid and breachPosition then
				...
				breachDestination = nextTargetPosition +
					flatFromToNextTarget:normalize() * (breachDepth + self.unit.character:getRadius())
...
function FarmbotUnit.server_onProjectile(self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal,
										 projectileUuid)
...
function FarmbotUnit.server_onCollision(self, other, collisionPosition, selfPointVelocity, otherPointVelocity,
										collisionNormal)
	if not sm.exists(self.unit) then
		return
	end
	...
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 then
			...
			startTumble(self, self.destroyedEventState.time * 40, self.destroyedEventState,
				self.unit.character.velocity + sm.vec3.new(0, 0, 2.5))
		else
			self.storage:save(self.saved)
		end
	end
...
function FarmbotUnit.sv_onDeath(self)
	...
	if not self.destroyed then
		...
		-- Create explosion
		sm.physics.explode(character.worldPosition, 7, 2, 6, 25, "Farmbot - Destroyed", nil,
			{ Color = self.unit.character:getColor() })
