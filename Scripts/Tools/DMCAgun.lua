...
local renderablesTp = { "$GAME_DATA/Character/Char_Male/Animations/char_male_tp_spudgun.rend",
...
function DMCAgun.client_onUpdate(self, dt)
	...
	self.tool:updateJoint("jnt_hips", sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		0.35 * finalJointWeight * (normalWeight))

	local crouchSpineWeight = (0.35 / 3) * crouchWeight

	self.tool:updateJoint("jnt_spine1", sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		(0.10 + crouchSpineWeight) * finalJointWeight)
	self.tool:updateJoint("jnt_spine2", sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		(0.10 + crouchSpineWeight) * finalJointWeight)
	self.tool:updateJoint("jnt_spine3", sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
...
function DMCAgun.cl_onSecondaryUse(self, state)
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend(sm.localPlayer.getInventory(), obj_plantables_potato, 1) then
			...
			local spreadDeg = fireMode.spreadMinAngle +
			(fireMode.spreadMaxAngle - fireMode.spreadMinAngle) * spreadFactor

			dir = sm.noise.gunSpread(dir, spreadDeg)

			local owner = self.tool:getOwner()
			if owner then
				sm.projectile.projectileAttack(sm.uuid.new("6bb9aa22-8b36-4fed-a740-028e5360a617"), 50, firePos,
					dir * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf)
			end

			-- Timers
			self.fireCooldownTimer = fireMode.fireCooldown
			self.spreadCooldownTimer = math.min(self.spreadCooldownTimer + fireMode.spreadIncrement,
				fireMode.spreadCooldown)
		...
		if #areaContents ~= 0 or areaContents ~= {} and areaContents ~= nil then
			for _, character in pairs(areaContents) do
				...
				if not character:isPlayer() then
					if not sm.exists(character) then return end
					sm.projectile.projectileAttack(sm.uuid.new("132c44d3-7436-419d-ac6b-fc178336dcb7"), 1,
						character.worldPosition + sm.vec3.new(0, 0, 0.5), sm.vec3.new(0, 0, 1), self.tool:getOwner())
				end
			end
		end
	end
