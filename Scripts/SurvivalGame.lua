...
function SurvivalGame.bindChatCommands(self)
	...
	if addCheats then
		...
		sm.game.bindChatCommand("/collisiondestruction", {}, "cl_onChatCommand", "Switches collision destruction on and off")
		sm.game.bindChatCommand("/ammoconsumption", {}, "cl_onChatCommand", "Switches ammo consumption on and off")
		sm.game.bindChatCommand("/fuelconsumption", {}, "cl_onChatCommand", "Switches fuel consumption on and off")
		sm.game.bindChatCommand("/partupgrade", {}, "cl_onChatCommand", "Switches ability to upgrade on and off")
	end
end
...
function SurvivalGame.cl_onChatCommand(self, params)
	...
	--E mod commads start here
	elseif params[1] == "/switchHUD" then
		sm.event.sendToScriptableObject(self.sv.HUDManager, "cl_switchHUD")
	elseif params[1] == "/collisiondestruction" then
		sm.event.sendToWorld(sm.localPlayer.getPlayer().character:getWorld(), "cl_switchDestruction")
...
function SurvivalGame.sv_onChatCommand(self, params, player)
	...
	elseif params[1] == "/ammoconsumption" then
		sm.game.setEnableAmmoConsumption(not sm.game.getEnableAmmoConsumption())
		self:client_showMessage("AMMO CONSUMPTION IS " .. (sm.game.getEnableAmmoConsumption() and "ON" or "OFF"))
	elseif params[1] == "/fuelconsumption" then
		sm.game.setEnableFuelConsumption(not sm.game.getEnableFuelConsumption())
